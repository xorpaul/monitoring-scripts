...
if disable_perfdata
  ...
  time.each {|k,v| perfdata_time += " run_time_#{k}=#{v}s" if k != 'last_run'}
  if changes 
...
elsif failcount_resources > 0
  puts "CRITICAL: Puppet last ran had #{failcount_resources} failed resources #{failcount_events} failed events#{perfdata_time}"
  exit 2

elsif failcount_resources > 0
  puts "WARNING: Puppet last ran had #{failcount_resources} failed resources #{failcount_events} failed events#{perfdata_time}"
  exit 1
